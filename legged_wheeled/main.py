...
env.cfg(control_if=args.control_if)

torch.manual_seed(args.seed)
np.random.seed(args.seed)    # the st seed, the same number of seed, the random is same

# Agent
# observation_space是Agent的观察空间
# 观察空间可以理解为Agent能够观察到的环境状态，读取有多少个状态
# action_space是Agent的动作空间，动作空间可以理解为Agent能够执行的动作，读取有多少个动作
...
while episode_steps < 6000 or average_reward < 3 or not end:
    ...
    while not done:
        # 在未达到start_steps之前，随机采样
        if args.start_steps > total_numsteps:
            action = env.action_space.sample()  # Sample random action;sample()方法是从一个均匀分布[low, high)中随机采样，返回一个样本值
        else:
            action = agent.select_action(state)  # Sample action from policy
        env.render()            # render()是渲染当前环境的方法
        # 如果还未达到经验池大小
