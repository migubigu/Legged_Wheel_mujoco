...
class BipedEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def is_healthy(self):
        ...
        is_healthy = (self.get_body_com("base_link")[2] > min_z) and (not self.bump_base()) and (abs(ang_yx[0]) < self.ang_range) and (min(abs(ang_yx[1]),abs(ang_yx[1] - np.pi),abs(ang_yx[1] + np.pi)) < self.ang_range)
        return is_healthy
    ...
    def step(self, action):
        ...
        reward = 14*r_vel + r_vel_z*3 + r_torque*2 + r_gravity*2 + r_ang_xy*2 + r_similar_legged*2 + r_stand*0.2 + self.healthy_reward*6 + r_height*1 + r_energy*2                           # 定义奖励函数
...
# reward函数
    ...
    def _reward_similar_legged(self):
        left_leg = torch.tensor(self.position[7:10])
        right_leg = torch.tensor(self.position[10:])
