...
class BipedEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        rewards_info['r_similar_legged'] = self._reward_similar_legged().cpu()*0.8
...
# reward函数
    ...
    # # 奖励转向时轮子转动的速度差
    # def _reward_ang_vel_wheel(self):
    #     wheel_ang_vel = torch.tensor([self.velocity[8], self.velocity[-1]])
    #     wheel_ang_vel_diff = wheel_ang_vel[0] - wheel_ang_vel[1]
    #     r = abs(wheel_ang_vel_diff * 0.2407 + self.commands[1])
    #     if r < 0.2:
    #         return torch.exp(-0.1*r)
    #     else:
    #         return torch.exp(-r)
