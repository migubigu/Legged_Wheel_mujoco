...
class BipedEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def cfg(self, control_if=False, cuda_if=False):
        ...
        self.stage_reward_thresholds = [4.0, 5.5, 6.5] # 提升到下一阶段所需的平均奖励阈值 (需要根据奖励函数实际调整)
        self.stage_episodes_stable = 10 # 需要连续多少个episode满足阈值才能升级
        self.stable_episode_count = 0
        print(f"Curriculum initialized at Stage {self.curriculum_stage}")
    ...
    def step(self, action):
        ...
        if self.curriculum_stage == self.max_curriculum_stage:
            r_vel_weight = 1.6
        r_vel = self._reward_tracking_vel()* r_vel_weight
        r_vel_z = self._reward_lin_vel_z()*0.2
        r_ang_xy = self._reward_ang_vel_xy()*0.2
        r_torque = self._reward_torque()*0.3          # 由于上下坡可能需要更高的力矩，因此暂时注释
        r_gravity = self._reward_gravity()*0.3
        r_similar_legged = self._reward_similar_legged()*0.2
        r_stand = self._reward_nominal_state()*0.08
        r_height = self._reward_base_height()*0.1
        r_energy = self._reward_energy()*0.2
        r_elur = self._reward_base_ang_xy()*0.4
        r_smooth = self._reward_action_smoothness()*0.2
        r_wheel_contect = self._reward_wheel_contact()*0.3
        r_ang_vel_enhance = self._reward_ang_vel_enhance()*r_vel_weight
        
        reward = r_vel + r_vel_z + r_torque + r_gravity + r_ang_xy + r_similar_legged + r_stand + self.healthy_reward*0.5 + r_height + r_energy + r_elur + r_smooth + r_wheel_contect + r_ang_vel_enhance                           # 定义奖励函数
...
# reward函数
    ...
    def _reward_base_height(self):
        ...
        print(torch.tensor(self.get_body_com("base_link")[2])-average_wheel)
    ...
    def _reward_ang_vel_enhance(self):
        ...
        if ang_vel_error_abs > 0.2:
            return torch.tensor(-1.5*ang_vel_error_abs)
        else:
